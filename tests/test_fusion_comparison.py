...
from src.fusion_trainer import _build_retrieval_features
...
def precompute_all_data(
    ...
    # Prepare data
    # CSVLabeledLoader already renamed 'claim' -> 'text' if needed
    texts = test_df["text"].tolist()
    gold_evidences = test_df["evidence"].tolist()
    # CSVLabeledLoader already normalized labels to int, no need to call _normalize_label
    ...
    for i in tqdm(range(0, len(texts), batch_size), desc="Pre-computing data"):
        ...
        with torch.inference_mode():
            with torch.autocast(
                device_type="cuda" if device == "cuda" else "cpu", dtype=amp_dtype
            ):
    ...
    # Convert to tensors
    retrieval_features_tensor = torch.tensor(
...
def test_fusion_fast(
    ...
    llm_logits = (
        precomputed_data["llm_logits_retrieved"]
...
def test_lora_fast(
    precomputed_data: Dict, evidence_mode: str, batch_size: int = 32
) -> Dict:
    """
...
def main():
    ...
    logger.info(f"\n{'#' * 80}")
    logger.info("# OPTIMIZED MODEL COMPARISON TEST SUITE")
    logger.info(f"{'#' * 80}")
    ...
    # Load test data using CSVLabeledLoader (same as training)
    # This properly parses evidence from JSON list format
    logger.info("Loading test dataset...")
    test_df = CSVLabeledLoader(TEST_DATA_PATH).load()
    ...
    # Build knowledge base from EVIDENCE ARTICLES (not claims!)
    # This matches the training approach in train_fusion.py
    ...
    for idx, row in test_df.iterrows():
        # Split evidence into individual articles (same as training)
        evidence_str = str(row["evidence"])
        articles = evidence_str.split("|||")

        for article in articles:
            article = article.strip()
            if len(article) > 10:  # Filter out empty or very short strings
                # Use incrementing timestamps to spread out
                base_time = datetime(2024, 1, 1, 0, 0, 0, tzinfo=timezone.utc)
                timestamp = base_time.replace(
                    hour=article_count % 24, day=1 + (article_count // 24) % 28
                )

                knowledge_base.append(
                    {
                        "text": article,  # Use evidence article, not claim
                        "timestamp": timestamp,
                    }
                )
                article_count += 1
    ...
    logger.info(
        f"✓ Built knowledge base: {len(knowledge_base)} evidence articles from {len(test_df)} samples"
    )
    ...
    # Test 2: Fusion + Gold
    result = test_fusion_fast(
        precomputed_data, fusion, retrieval_encoder, "gold", DEVICE, EVAL_BATCH_SIZE
    )
    all_results.append(result)

    # Test 3: LoRA + Retrieved
    result = test_lora_fast(precomputed_data, "retrieved", EVAL_BATCH_SIZE)
    all_results.append(result)

    # Test 4: LoRA + Gold
    result = test_lora_fast(precomputed_data, "gold", EVAL_BATCH_SIZE)
    all_results.append(result)
